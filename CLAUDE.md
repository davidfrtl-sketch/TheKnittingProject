...
- **Fórmula universal de reparto de forma** (entallado, manga): `cadencia_primaria = techo(filas disponibles ÷ eventos de cambio)`, con una minoría de eventos ejecutados a `cadencia_primaria - 1` (más frecuente) para que la suma cierre exacto contra las filas disponibles — no hay resto que absorber en filas lisas sueltas.
